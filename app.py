...
**v1.5 — «Связи» (lead/lag).** Корреляции считаем ТОЛЬКО на стационарных рядах (годовые
темпы), скользящим окном (12 мес) и кросс-корреляцией (лаг ±6 мес). Тестируем лишь
ЗАРАНЕЕ заданные гипотезы — перебор всех пар не делаем (это p-hacking). Точек после 2022
мало → мощность низкая, результат **индикативный**. Корреляция ≠ причинность; в военной
экономике многое движется общим фискально-военным драйвером (совместное движение ≠ связь).

**v1.5 — «Своя корзина» и часть линз — ручной seed.** Средние цены Росстата (fedstat 31448,
коды товаров — в `data/seed/basket.csv`), PPI, базовый ИПЦ, дефлятор, Urals, топливо,
Ромир/Чек Индекс: программный доступ к ЕМИСС/проприетарным источникам из окружения сборки
недоступен → по дизайну PRD это seed-CSV, заполняются вручную из первоисточника. Что не
заполнено — честно помечено «ожидает данных», а не выдумано. **Алерты** — правиловые,
информационные (не прогноз), пороги по собственной истории/значению с антидребезгом.

**Источники:** Банк России (ставка, деньги, курс, ИПЦ, ИнФОМ), Минфин (бюджет, ФНБ),
Росстат (ИПЦ, средние цены, реальный ВВП). Каждый график несёт источник, единицы и дату
обновления в тултипе.
"""
        )
